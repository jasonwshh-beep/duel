...
const ROUND_MS = Number(process.env.ROUND_MS || 500);
const DUEL_DURATION_MS = Number(process.env.DUEL_DURATION_MS || 15000);
...
function rollTwo(){
  ...
  const winner = winnerSide === 1 ? a : b;
  const loser = winnerSide === 1 ? b : a;
  const winnerEndHp = Math.floor(18 + Math.random() * 45);
  const now = Date.now();
  ...
  const duel = {
    ...
    maxHp: MAX_HP,
    startedAt: now,
    endsAt: now + DUEL_DURATION_MS,
    durationMs: DUEL_DURATION_MS,
    winnerSide,
    plannedWinner: winner,
    plannedLoser: loser,
    winner: null,
    loser: null,
    log: [`${a} and ${b} enter the arena!`]
  };
...
function startBattle(id){
  const attackLines = [
    'slashes through the guard',
    'lands a heavy sword strike',
    'counters with a shield bash',
    'charges across the arena',
    'swings for a critical hit',
    'forces their opponent back'
  ];
  const timer = setInterval(() => {
    ...
    const now = Date.now();
    const progress = Math.min(1, Math.max(0, (now - d.startedAt) / d.durationMs));
    const ease = 1 - Math.pow(1 - progress, 1.35);
    const winnerEndHp = d._winnerEndHp || (d._winnerEndHp = Math.floor(18 + Math.random() * 45));

    if (d.winnerSide === 1) {
      d.hp1 = Math.max(winnerEndHp, Math.round(MAX_HP - (MAX_HP - winnerEndHp) * ease));
      d.hp2 = Math.max(0, Math.round(MAX_HP * (1 - ease)));
    } else {
      d.hp1 = Math.max(0, Math.round(MAX_HP * (1 - ease)));
      d.hp2 = Math.max(winnerEndHp, Math.round(MAX_HP - (MAX_HP - winnerEndHp) * ease));
    }

    if (!d._lastLogAt || now - d._lastLogAt > 1700) {
      d._lastLogAt = now;
      const attacker = Math.random() < 0.5 ? d.fighter1 : d.fighter2;
      const line = attackLines[Math.floor(Math.random() * attackLines.length)];
      d.log.unshift(`${attacker} ${line}!`);
      d.log = d.log.slice(0, 6);
    }

    if (progress >= 1) {
      d.status = 'finished';
      d.winner = d.plannedWinner;
      d.loser = d.plannedLoser;
      if (d.winnerSide === 1) { d.hp1 = winnerEndHp; d.hp2 = 0; }
      else { d.hp1 = 0; d.hp2 = winnerEndHp; }
      d.log.unshift(`${d.winner} wins the 15-second duel!`);
